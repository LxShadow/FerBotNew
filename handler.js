...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '𝑭𝒆𝒓 𝑩𝒐𝒕') :
...
"thumbnail": apii.data, 
"title": [wm, '❄ https://www.instagram.com/fer_shp11?igsh=bjBoczkyMm92MnZn'].getRandom(),
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '❄', 'https://www.instagram.com/fer_shp11?igsh=bjBoczkyMm92MnZn'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, channel1, channel2, nna, yt, ig, paypal, fb].getRandom() }}}}, tg)
